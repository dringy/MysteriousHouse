...
def get_move_response(osstate, x, y, flavour_text, mob_x, mob_y):
    ...
    # End Detection
        ...
        return  get_audio_response(
            ...
            "You have reached a checkpoint. Say orange to warp back to this point. "
...
def get_move_forward_response(osstate, x, y, mob_x, mob_y):
        if osstate <= 1: # north
            y+=1
        elif osstate == 2: # east
            x+=1
        ...
        return get_move_response(osstate, x, y, "You continue straight ahead. ", mob_x, mob_y)


def get_move_backward_response(osstate, x, y, mob_x, mob_y):
        directions = get_floor2_directions(osstate, x, y)
        if osstate <= 1:  # south
            y -= 1
            osstate = 3
        ...
        return get_move_response(osstate, x, y, "You turn around and go back. ", mob_x, mob_y)


def get_move_left_response(osstate, x, y, is_continue, mob_x, mob_y):
        directions = get_floor2_directions(osstate, x, y)
        if osstate <= 1:  # west
            x -= 1
            osstate = 4
        ...
        return get_move_response(osstate, x, y, flavour_text, mob_x, mob_y)


def get_move_right_response(osstate, x, y, is_continue, mob_x, mob_y):
        if osstate <= 1:  # east
            x += 1
            osstate = 2
...
def on_intent(intent_request, session):
    ...
    if intent_name == "AMAZON.StopIntent" or intent_name == "AMAZON.CancelIntent":
        ...
        return get_start_response()
    elif intent_name == "CheckpointOneIntent":
    ...
    elif intent_name == "CheckpointTwoIntent":
        return get_audio_response(
            get_starting_floor3_attributes(),
            "What do you choose?",
    ...
    if floor == 1:
        return on_intent_floor1(intent_name, session)
    elif floor == 2:
        return on_intent_floor2(intent_name, session)
    elif floor == 3:
        return on_intent_floor3(intent_name, session)
    else:
        return  get_start_response()


def on_intent_floor1(intent_name, session):
    ...
    if intent_name == "AMAZON.HelpIntent" or intent_name == "AMAZON.RepeatIntent" or intent_name == "PlayIntent":
        ...
        if asking_larry:
            return get_doubleaudio_response(
                get_starting_floor2_attributes(),
                "Down the Ladder.",
                "Larry seems surprised but lets you climb down the ladder anyway. You have reached a checkpoint. "
                "Say Limestone to warp back to this point. "
...
def on_intent_floor2(intent_name, session):
    ...
    if intent_name == "AMAZON.RepeatIntent" or intent_name == "PlayIntent":
        ...
        directions = get_floor2_directions(osstate, x, y)
        if directions[0]:
            return get_move_forward_response(osstate, x, y, mob_x, mob_y)
        else:
            movement_options = get_floor2_movement_options_state(osstate, x, y)
            return get_response(
                construct_floor2_attributes(x, y, osstate, mob_x, mob_y),
        ...
        directions = get_floor2_directions(osstate, x, y)
        if directions[1]:
            return get_move_backward_response(osstate, x, y, mob_x, mob_y)
        else:
            movement_options = get_floor2_movement_options_state(osstate, x, y)
            return get_response(
                construct_floor2_attributes(x, y, osstate, mob_x, mob_y),
        ...
        directions = get_floor2_directions(osstate, x, y)
        if directions[2]:
            return get_move_left_response(osstate, x, y, intent_name == "ContinueLeftIntent", mob_x, mob_y)
        else:
            movement_options = get_floor2_movement_options_state(osstate, x, y)
            return get_response(
                construct_floor2_attributes(x, y, osstate, mob_x, mob_y),
        ...
        directions = get_floor2_directions(osstate, x, y)
        if directions[3]:
            return  get_move_right_response(osstate, x, y, intent_name == "ContinueRightIntent", mob_x, mob_y)
        else:
            movement_options = get_floor2_movement_options_state(osstate, x, y)
            return  get_response(
                construct_floor2_attributes(x, y, osstate, mob_x, mob_y),
        ...
        if (not directions[0]) and (not directions[2]) and directions[3]:
            return get_move_right_response(osstate, x, y, True, mob_x, mob_y)

        # Left Only (except back)
        elif (not directions[0]) and directions[2] and (not directions[3]):
            return get_move_left_response(osstate, x, y, True, mob_x, mob_y)
...
def on_intent_floor3(intent_name, session):
    ...
    # Cake
        ...
        return get_audio_response(
            ...
            "doomed to walk these halls forever. The End, Thank you for playing!",
            jingle_sound(),
            "",
            None,
            True # End Game Here
        ...
        return get_audio_response(
            ...
            "You are wearing a nametag, it says Larry. The End, Thank you for playing!",
            jingle_sound(),
            "",
            None,
            True  # End Game Here
